<template>
  <div id="designer">
    ...
    <div id="toolbar">
      <div
        @click="switchMode(enums.mode.select)"
        :class="{active:tempData.mode.current==enums.mode.select}"
      >
        <p>选择</p>
      </div>
      <div
        @click="switchMode(enums.mode.connect)"
        :class="{active:tempData.mode.current==enums.mode.connect}"
      >
        <p>连接</p>
      </div>
      <hr />
      <div
        @click="switchMode(enums.mode.addNode_task)"
        :id="enums.mode.addNode_task"
        :nodetype="enums.nodeType.task"
        :class="{active:tempData.mode.current==enums.mode.addNode_task}"
      >
        <p>任务</p>
      </div>
      <div
        @click="switchMode(enums.mode.addNode_parallel)"
        :id="enums.mode.addNode_parallel"
        :nodetype="enums.nodeType.task"
        :class="{active:tempData.mode.current==enums.mode.addNode_parallel}"
      >
        <p>分支</p>
      </div>
      <div
        @click="switchMode(enums.mode.addNode_merge)"
        :id="enums.mode.addNode_merge"
        :nodetype="enums.nodeType.task"
        :class="{active:tempData.mode.current==enums.mode.addNode_merge}"
      >
        <p>合并</p>
      </div>
      <div
        @click="switchMode(enums.mode.addNode_subflow)"
        :id="enums.mode.addNode_subflow"
        :nodetype="enums.nodeType.task"
        :class="{active:tempData.mode.current==enums.mode.addNode_subflow}"
      >
        <p>子流程</p>
      </div>
    </div>
    <div id="container">
      <!-- 画布 -->
      <svg
        :width="flowData.paperWidth"
        @keyup.46="remove"
        @keyup.ctrl.67.exact="copy"
        @keyup.ctrl.86.exact="paste"
        @keyup.ctrl.90.exact="undo"
        tabindex="0"
        :height="flowData.paperHeight"
        id="paper"
        version="1.1"
      ...
      >
        <defs>
          ...
          >
            <path stroke="none" class="unselect" transform="rotate(180)" d="M 10 -5 0 0 10 5 z" />
          </marker>
          <marker id="arrow-select" orient="auto" overflow="visible" markerUnits="userSpaceOnUse">
            <path stroke="none" class="select" transform="rotate(180)" d="M 10 -5 0 0 10 5 z" />
          </marker>
        </defs>
        <!-- 节点 -->
        <g
          v-for="item in flowData.nodes"
          :key="'node'+item.id"
          :id="item.id"
          cursor="pointer"
          @dblclick="nodeDblClick(item)"
          @mousedown.stop="beginMove"
          @mouseup="select('node',item.id,$event)"
          :class="tempData.currentSelect.type=='node'&&tempData.currentSelect.id==item.id?'select':'unselect'"
        >
          <title>{{item.text}}</title>
          {{ shortText = item.text.substringIfTooLong(6) }}
          <TaskNode
            v-if="item.type==enums.nodeType.task"
          ...
          >{{shortText}}</TaskNode>
          <StartNode
            v-if="item.type==enums.nodeType.start"
            :width="item.nodeWidth"
            :height="item.nodeHeight"
            :x="item.x"
            :y="item.y"
          >{{shortText}}</StartNode>
          <StopNode
            v-if="item.type==enums.nodeType.stop"
            :width="item.nodeWidth"
            :height="item.nodeHeight"
            :x="item.x"
            :y="item.y"
          >{{shortText}}</StopNode>
          <ParallelNode
            v-if="item.type==enums.nodeType.parallel"
            :width="item.nodeWidth"
            :height="item.nodeHeight"
            :x="item.x"
            :y="item.y"
          >{{shortText}}</ParallelNode>
          <MergeNode
            v-if="item.type==enums.nodeType.merge"
            :width="item.nodeWidth"
            :height="item.nodeHeight"
            :x="item.x"
            :y="item.y"
          >{{shortText}}</MergeNode>
          <SubflowNode
            v-if="item.type==enums.nodeType.subflow"
            :width="item.nodeWidth"
            :height="item.nodeHeight"
            :x="item.x"
            :y="item.y"
          >{{shortText}}</SubflowNode>
        </g>
        <!-- 连线 -->
        <g
          v-for="item in flowData.lines"
          :key="'line'+item.id"
          cursor="pointer"
        ...
        >
          ...
          <path
            ...
            fill="none"
            stroke-width="2"
            :marker-end="tempData.currentSelect.type=='line'&&tempData.currentSelect.id==item.id?'url(#arrow-select)':'url(#arrow-unselect)'"
          />
          <text :x="lineData.textx" :y="lineData.texty">{{item.text.substringIfTooLong(12)}}</text>
        </g>
        <!-- 动态连线 -->
        <path
          :d="tempData.connectLine.path"
          v-if="tempData.connectLine.path"
          fill="none"
          class="unselect"
          stroke-width="2"
          marker-end="url(#arrow-unselect)"
        />
      </svg>
    </div>
...
import TaskNode from "@/components/common/workflownodes/task.vue";
import StartNode from "@/components/common/workflownodes/start.vue";
import StopNode from "@/components/common/workflownodes/stop.vue";
import ParallelNode from "@/components/common/workflownodes/parallel.vue";
import MergeNode from "@/components/common/workflownodes/merge.vue";
import SubflowNode from "@/components/common/workflownodes/subflow.vue";

commonfun.useExtends();
let mode = {
    // 操作模式
    select: "select", // 选择
    connect: "connect", // 连接
...
export default {
  ...
  methods: {
    ...
    drop(ev) {
      ...
      else if (this.tempData.mode.current == mode.connect) this.connectDrop(ev);
    },
    beginDrag(ev) {
      let node = this.getMousePointNode(ev);
      if (!node) console.error(ev);
      // 添加到更改历史
      this.tempData.historyData.push(commonfun.clone(this.flowData));
      // 开始拖动 记录拖动数据
    ...
    connectDrop(ev) {
      ...
      if (targetNode) {
        let exists = this.flowData.lines.some(
          r =>
            r.from == this.tempData.connectLine.nodeId && r.to == targetNode.id
        );
        if (!exists && this.tempData.connectLine.nodeId != targetNode.id) {
          // 添加到更改历史
          this.tempData.historyData.push(commonfun.clone(this.flowData));
    ...
    // 移除某个对象
    remove() {
      if (this.tempData.currentSelect.id && this.tempData.currentSelect.type) {
        // 添加到更改历史
        this.tempData.historyData.push(commonfun.clone(this.flowData));
    ...
    copy() {
      ...
      else this.tempData.clipboard.nodeId = null;
    },
    // 粘贴
    paste() {
      if (this.tempData.clipboard.nodeId) {
        ...
        let cloneNode = commonfun.clone(node);
        cloneNode.x += 20;
        cloneNode.y += 20;
        cloneNode.id = commonfun.guid();
        // 添加到更改历史
        this.tempData.historyData.push(commonfun.clone(this.flowData));
    ...
    undo() {
      if (this.tempData.historyData.length) {
        ...
        if (last) {
          this.flowData = commonfun.clone(last);
